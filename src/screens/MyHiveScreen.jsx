...
export default function MyHiveScreen({
    ...
    return (
      <div style={{ minHeight: '100%', background: '#f4ecd8' }}>
        <div style={{ padding: '42px 24px 12px', position: 'sticky', top: 0, zIndex: 20, background: '#f4ecd8', borderBottom: '1px solid #c8b890' }}>
          ...
          <p style={{ margin: 0, fontSize: 13, color: '#6b5838', fontFamily: 'var(--theme-font-head)' }}>
            {t(s.myhive_subtitle)}
          </p>
        </div>
        <div style={{ padding: '16px 24px 80px' }}>
          ...
          <ColoniesSection colonies={profile?.colonies ?? []} onAdd={onAddColony} onUpdate={onUpdateColony} onRemove={onRemoveColony} />
  ...
  if (theme === 'c') {
    const colonyCount = profile?.colonies?.length ?? 0
    return (
