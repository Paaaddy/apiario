...
export default function SeasonHeader({ theme, selectedDate, icon, label, week, rangeLabel, taskCount, viewingToday, locale, t, onPrev, onNext, onToday }) {
    ...
    return (
      <>
        <div style={{ position: 'sticky', top: 0, zIndex: 20, padding: '10px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: pal.sun + 'e8', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)' }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: c.ink, letterSpacing: 1, textTransform: 'uppercase', display: 'flex', gap: 6, alignItems: 'center' }}>
        ...
        <div style={{ position: 'relative', overflow: 'hidden', minHeight: 220 }}>
          ...
          <div style={{ position: 'relative', padding: '12px 22px 24px', zIndex: 2 }}>
            <h1 style={{ margin: 0, fontFamily: '"Playfair Display", serif', fontSize: 58, lineHeight: 0.92, fontWeight: 700, color: c.ink, letterSpacing: -1 }}>
              {viewingToday
            ...
            <p style={{ margin: '12px 0 0', fontSize: 13, color: c.ink, maxWidth: '80%' }}>
              {rangeLabel} · {taskCount} {t(s.season_tasks_noun)}
            </p>
          </div>
        </div>
        <div style={{ padding: '0 14px 8px', display: 'flex', alignItems: 'center', gap: 10 }}>
          <button type="button" onClick={onPrev} aria-label={t(s.week_previous)} style={{ width: 34, height: 34, borderRadius: '50%', background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(28,20,16,0.12)', color: c.ink, fontSize: 16, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>‹</button>
          <div style={{ flex: 1 }} />
          {!viewingToday && (
            <button type="button" onClick={onToday} style={{ fontSize: 11, fontWeight: 600, color: c.accentDark, textDecoration: 'underline', background: 'none', border: 'none', cursor: 'pointer' }}>{t(s.week_today)}</button>
          )}
          <button type="button" onClick={onNext} aria-label={t(s.week_next)} style={{ width: 34, height: 34, borderRadius: '50%', background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(28,20,16,0.12)', color: c.ink, fontSize: 16, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>›</button>
        </div>
      </>
    )
  }
  ...
  return (
    <div style={{ padding: '42px 24px 12px', position: 'sticky', top: 0, zIndex: 20, background: c.bg, borderBottom: `1px solid ${c.rule}` }}>
      ...
      <h1 style={{ margin: '14px 0 4px', fontFamily: 'var(--theme-font-head)', fontSize: 42, lineHeight: 0.95, fontWeight: 400, color: c.ink, letterSpacing: -0.5 }}>
        {icon}{viewingToday
      ...
      <p style={{ margin: '4px 0 0', fontSize: 13, color: c.inkMid, fontFamily: 'var(--theme-font-head)' }}>
        {taskCount} {t(s.season_tasks_noun)}
      </p>
      <div style={{ marginTop: 12, padding: '8px 0', borderTop: `1px solid ${c.rule}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <button type="button" onClick={onPrev} aria-label={t(s.week_previous)} style={{ fontFamily: 'var(--theme-font-mono)', fontSize: 11, color: c.inkMid, letterSpacing: 1, background: 'none', border: 'none', cursor: 'pointer' }}>‹ {t(s.week_previous)}</button>
        {!viewingToday && (
          <button type="button" onClick={onToday} style={{ fontSize: 11, fontWeight: 600, color: c.accent, textDecoration: 'underline', background: 'none', border: 'none', cursor: 'pointer' }}>{t(s.week_today)}</button>
        )}
        <button type="button" onClick={onNext} aria-label={t(s.week_next)} style={{ fontFamily: 'var(--theme-font-mono)', fontSize: 11, color: c.inkMid, letterSpacing: 1, background: 'none', border: 'none', cursor: 'pointer' }}>{t(s.week_next)} ›</button>
      </div>
    </div>
  )
}
